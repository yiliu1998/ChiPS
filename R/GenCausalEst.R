...
.point.est <- function(A, Y, X,
  ...
  # step 1: propensity score estimation via logistic regression
  X <- as.data.frame(X)
  colnames(X) <- paste0("X", 1:ncol(X))
  fit <- glm(A~.-A, data=data.frame(A, X), family=binomial(link="logit"))
  e.h <- predict(fit, X, type="response")

  # step 2: define the tilting functions
  ...
  # step 3: calculate the propensity score weighted estimator for general estimands
  M <- ncol(tilt.wate)
